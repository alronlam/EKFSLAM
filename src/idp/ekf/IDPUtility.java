...
public class IDPUtility {
	...
	private static Matrix hi_inverse_depth(IDPFeature yinit, Matrix t_wc, Matrix r_wc, Camera cam,
			ArrayList<FeatureInfo> features_info) {
	...
	private static Matrix distort_fm(Matrix uv, Camera cam) {
		...
		double[][] out = { { xu / D / cam.dx + cam.Cx, yu / D / cam.dy + cam.Cy } };
	...
	private static Matrix calculate_Hi_inverse_depth(Matrix Xv_km1_k, Matrix yi, Camera cam, int index,
			ArrayList<FeatureInfo> features_info) {
	...
	private static Matrix dh_dxv(Camera cam, Matrix Xv_km1_k, Matrix yi, Matrix zi) {
		...
		// Hi1 = [ dh_drw( camera, Xv_km1_k, yi, zi ) dh_dqwr( camera, Xv_km1_k,
		// yi, zi ) zeros( 2, 6 )];
	...
	private static Matrix dhrl_dqwr(Matrix Xv_km1_k, Matrix yi) {
		...
		// a = dRq_times_a_by_dq( qconj(qwr), ((yi(1:3) - rw)*lambda + mi)
		// )*dqbar_by_dq;

		Matrix a = dRq_times_a_by_dq(QuaternionHelper.qconj(qwr),
				(yi.getMatrix(0, 2, 0, 0).minus(rw).times(lambda).plus(mi)));
	...
	private static Matrix dR_by_dq0(Matrix q) {
		...
		double[][] out = { { 2 * q0, -2 * qz, 2 * qy }, { 2 * qz, 2 * q0, -2 * qx }, { -2 * qy, 2 * qx, 2 * q0 } };
	...
	private static Matrix dR_by_dqx(Matrix q) {
		...
		double[][] out = { { 2 * qx, 2 * qy, 2 * qz }, { 2 * qy, -2 * qx, -2 * q0 }, { 2 * qz, 2 * q0, -2 * qx } };
	...
	private static Matrix dR_by_dqy(Matrix q) {
		...
		double[][] out = { { -2 * qy, 2 * qx, 2 * q0 }, { 2 * qx, 2 * qy, 2 * qz }, { -2 * q0, 2 * qz, -2 * qy } };
	...
	private static Matrix dR_by_dqz(Matrix q) {
		...
		double[][] out = { { -2 * qz, -2 * q0, 2 * qx }, { 2 * q0, -2 * qz, 2 * qy }, { 2 * qx, 2 * qy, 2 * qz } };
	...
	private static Matrix jacob_undistor_fm(Camera cam, Matrix uvd) {
		...
		double[][] a = {
				{
						(1 + cam.k1 * rd2 + cam.k2 * rd4) + (ud - cam.Cx) * (cam.k1 + 2 * cam.k2 * rd2)
								* (2 * (ud - cam.Cx) * cam.dx * cam.dx),
						(ud - cam.Cx) * (cam.k1 + 2 * cam.k2 * rd2) * (2 * (vd - cam.Cy) * cam.dy * cam.dy) },
				{
						(vd - cam.Cy) * (cam.k1 + 2 * cam.k2 * rd2) * (2 * (ud - cam.Cx) * cam.dx * cam.dx),
						(1 + cam.k1 * rd2 + cam.k2 * rd4) + (vd - cam.Cy) * (cam.k1 + 2 * cam.k2 * rd2)
