...
public class MainDriver {
	...
	public static void main(String[] args) {
		...
		// runAsync(imuDataset, imgDataset, asyncLogFileName, false);
		// runAsync(imuDatasetWithCimuHeading, imgDataset, asyncLogFileName,
		// false);
	...
	private static void runVINSAsync(List<IMUReadingsBatch> imuDataset, List<Mat> imgDataset, String logFileName,
		...
		while (true) {
			...
			if (elapsedTime >= Constants.MS_IMU_DURATION) {
				...
				/* Image Update */
				if (prevPoint.getX() != predictResult.getX() || prevPoint.getY() != predictResult.getY()) {
					FeatureUpdate featureUpdate = featureManager.getFeatureUpdate(imgDataset.get(imgIndex), transX,
